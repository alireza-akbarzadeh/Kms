...
import MyDocumnetTableBody from "./MyDocumnetTableBody";
...
const BookmarkIndex = () => {
  ...
  const { data, loading } = useSelector((state) => state.ownIndexDocumnetSlice);
  ...
  React.useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (searchTerm !== "") {
        const res = await Http(`document/index/own?search=${searchTerm}`, {
          method: "get",
        });
        if (res.status === 200) {
  ...
  const handlePageChange = async (event, value) => {
    ...
    const res = await Http(
      `document/index/own?page=${page}&perPage=${perPage}`,
      {
        method: "get",
      }
    );
  ...
  const handleFilterChange = async (queryString) => {
    const res = await Http(`document/index/own?${queryString}`, {
      method: "get",
    });
    if (res.status === 200) {
      setTemplateClone(res?.data?.data);
    ...
    <>
      <Box mt={3}>
        <Container maxWidth="xl">
          ...
          >
            <Typography sx={{ color: "#fff" }}>{t("myDocs")}</Typography>
            <Box>
              <Input
          ...
          {template?.data?.data?.length > 0 ? (
            <TableContainer sx={{ border: border }} component={Paper}>
              <Table sx={{ minWidth: 650 }} aria-label="simple table">
                ...
                <TableBody>
                  <MyDocumnetTableBody data={template?.data?.data} />
                </TableBody>
              </Table>
