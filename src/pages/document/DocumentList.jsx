...
import {Archive, ArchiveOutlined, ArrowLeft, Edit, RestoreOutlined} from "@mui/icons-material";
...
import {recoverCategoryList} from "../../redux/features/document/Category/delete/recoverCategory";
...
const DocumentList = ({id, data, loading, handleFolderChange, archive = false}) => {
    ...
    const handlePageChange = async (event, value) => {
        setPage(value);
        let page = value;
        const res = await Http(`document/category/${1}?page=${page}&perPage=${perPage}`, {
            method: "get",
        });
        if (res.status === 200) {
                ...
                <Box sx={{display: "inline-flex", alignItems: "center", margin: '0 10px'}}>
                    <Stack sx={{flexWrap: "wrap"}} spacing={2}>
                        ...
                        {!archive ? (
                            ...
                            <RestoreOutlined onClick={() => restoreCategory(item)} sx={{cursor: 'pointer'}}/>
                        )}
                    </Stack>
    ...
    const restoreCategory = (item) => {
        dispatch(recoverCategoryList(item.id))
            .then(res => {
                handleFolderChange(id, id, data?.parent?.type)
            })
    }
