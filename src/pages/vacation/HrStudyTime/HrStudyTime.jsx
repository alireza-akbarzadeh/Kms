...
import HrStduyTableBody from "./HrStduyTableBody";
...
const HrStudyTime = () => {
    ...
    React.useEffect(() => {
        dispatch(getHrStudyTime({id: null}));
    }, [dispatch]);
    ...
    const handlePageChange = async (event, value) => {
        ...
        const res = await Http(
            `user/hr/logIndex/studyTime/index?page=${page}&perPage=${perPage}`,
            {
                method: "get",
            }
        );
    ...
    React.useEffect(() => {
        const delayDebounceFn = setTimeout(async () => {
            if (searchTerm !== "") {
                const res = await Http(
                    `user/hr/logIndex/studyTime/index?search=${searchTerm}`,
                    {
                        method: "get",
                    }
                );
    ...
    const handleFilterChange = async (queryString) => {
        const res = await Http(
            `user/hr/logIndex/studyTime/index?search=${queryString}`,
            {
                method: "get",
            }
        );
